...
void TCPServerWidget::PrintState(QByteArray package)
{
    ...
    float battVolt = (float)(3.3 * 2 * rawBattVolt / 4096);           // //将原始测量值转换为实际的电压值，3.3是参考电压，2是因为使用了分压电路（5V电池，2个10k分压），4096是ADC的分辨率
    ...
    switch (devId) {
        case 1:{
            ...
            ui->label_BATT_VOLT->setText(QString::asprintf("%5.3f", battVolt)); // 电池电压,使用asprintf函数将电池电压格式化为字符串，保留3位小数
            break;
        }
        case 2:{
